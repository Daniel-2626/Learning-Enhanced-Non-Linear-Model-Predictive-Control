...
def plot_control_jacobians_wrt_w(model, T_bat_fixed, current_fixed, omega_scale, Q_heat_scale):
    ...
    t_in = torch.full_like(q_range, T_bat_fixed / 100.0)
    i_in = torch.full_like(q_range, current_fixed / 25.0)
    q_in = torch.full_like(q_range, 2) # Fixed Q heat at 2000 W
...
def main():
    ...
    omega_scaled_data = omega_scaled_data
    Q_heat_scaled_data = Q_heat_scaled_data
    dt = 5
    ...
    for t in range(n_rows - 1):
        ...
        T_bat = T_bat_pred  
    T_preds_nom = np.insert(T_preds_nom, 0, T_bat_scale*temperatures_data[0], axis=0)
    
    T_preds_nn = np.insert(T_preds_nn, 0, T_bat_scale*temperatures_data[0], axis=0)
