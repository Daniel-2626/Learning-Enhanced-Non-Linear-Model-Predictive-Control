...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim=2, output_dim=1, hidden_dim=128, num_layers=3):
        ...
        layers = [nn.Linear(input_dim, hidden_dim, bias=True), nn.Tanh()] 
        for _ in range(num_layers - 1):
            layers.extend([nn.Linear(hidden_dim, hidden_dim, bias=True), nn.Tanh()])
...
class BatteryLearnedDynamics:
    ...
    def model(self, T_env):
        ...
        # nn is off until first training (or if NN showcases poor performance on test data set)
    ...
    def optimization_problem_steady_state(self, dt, T_env):
        ...
        st_next = X[:, 1]
        # Not using NN in steady state selection 
        # But can use if uncomment following lines
        #mlp_input_K1 = cs.vertcat(st, disturbances[0], U[0], U[1]) 
        K1 = (f_model(st, U, disturbances[0]))/T_bat_scale # + disturbances[1] * self.residual_model(mlp_input_K1.T).T[0])/(T_bat_scale) <-- uncomment this to use NN in steady state selection     
        ...
        ug[1] = T_clin_max 
        lg[2] = T_clout_min # Even is T_clout
        ug[2] = T_clout_max
...
class Controller:
    ...
    def get_cost_to_go(self, T_bat_target, T_env, omega_norm_ss, Q_heat_norm_ss):
        ...
        # Once again, the residual NN is not used when calculating the cost-to-go
        # But can be used by uncommenting the following lines (and perhaps some testing/debugging)
        ...
        #T_bat_dot_model = T_bat_dot_model #+ T_bat_dot_residual <-- uncomment for using NN in cost-to-go
    ...
    def get_input(self, T_bat_target, T_bat_0, SOC_0, current_0, dT_bat, dSOC):
        ...
        print("--------------------------------")
        print("ADAPTIVE DERIVATIVE LOSS MPC BATTERY MODEL WITH TARGET TRACKING AND COST-TO-GO")
        start = time.time()
        T_bat_target = T_bat_target/self.T_bat_scale
        ...
        if (self.dt*self.current_iterate) > self.T_warm_start and ((self.dt*self.current_iterate) % self.T_update) == 0 and len(self.data) >= self.batch_size:
            ...
            # Estimated true dynamics (filtered)
            ...
            # Estimated model dynamics (filtered)
            ...
            confidence = 0.001
            # Confidence is hardcoded, but uncomment following if want to calculate automatically (not tested, probably poor performance)
            ...
            for _ in range(200): 
                ...
                loss.backward() 
                # Clipping to reduce gradients in NN
            ...
            for p in self.residual_mlp.parameters(): 
                p.requires_grad = False # Freeze parameters again
           
            # Update parameters in residual 
            ...
            self.residual_mlp.eval()
            # Predict residual on test data
            with torch.no_grad():
                outputs = self.residual_mlp(test_data)
                target_predicted = np.array(outputs.squeeze().tolist())
            rmse = root_mean_squared_error(y_target_test, target_predicted)
            len_test = len(y_test)
            # Null prediction (i.e. 0 residual)
            null_prediction = np.zeros((len_test,))
            print("RMSE", rmse)
            rmse_null_hypothesis = root_mean_squared_error(y_target_test, null_prediction)
            print("RMSE null hypothesis", rmse_null_hypothesis)
            self.residual_mlp.train()
            # Turn off NN if its RMSE is larger than null hypothesis
            if rmse >= rmse_null_hypothesis:
                self.nn_on = 0
