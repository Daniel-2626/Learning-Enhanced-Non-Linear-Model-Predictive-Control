...
class BatteryDynamics:
    ...
    def model(self, T_env): # remove gym_env for cascaded tank
        ...
        T_bat_dot_model = alpha_0/(m_battery*c_battery) * (((R_battery+theta_0) * current**2  - (1 + theta_1)*Q_cool) + (gamma + theta_2)*(T_env - T_bat))
        ...
        model.u = U
        model.z = cs.vertcat([])
        model.p = cs.vertcat(current, theta_0, theta_1, theta_2)
        model.f_expl = f_expl
...
class Controller:
    ...
    def get_input(self, T_bat_target, T_bat_0, SOC_0, current_0, dT_bat, dSOC):
        ...
        print("--------------------------------")
        print("LMS ACADOS MPC BATTERY MODEL")
        print("time", self.current_iterate*self.dt)
        ...
        Q_cool_eval = 0
        pred_error = np.array([[0]])
        # LMS update
        if (self.dt * self.current_iterate) > self.T_warm_start + 1:
            print(self.current_iterate * self.dt)
            T_bat_pred = self.xt_pred[0].item()
            T_bat_last = self.x_last[0].item()
            omega_last = self.omega_last
            Q_heat_last = self.Q_heat_last
            current_last = disturbances[int(self.dt*(self.current_iterate - 1))].item()
            print("time", self.current_iterate*self.dt)
            print("T_BAT_0", T_bat_0)
            print("T_bat_pred", T_bat_pred)
            ...
            current_sq = np.array([[current_last**2]]) #np.array([[0]]) 
            environment_losses = np.array([[T_env - T_bat_last]])
            #environment_losses = np.array([[0]])
            
            G = alpha_0/(m_battery*c_battery) * np.concatenate([current_sq, -Q_cool_eval, environment_losses])
            mu = 1
        ...
        for k in range(self.N):
            ...
            param_values = np.concatenate([np.array([[disturbances[int(self.dt*(self.current_iterate + k))].item()]]), self.theta])
        ...
        param_values = np.concatenate([np.array([[disturbances[int(self.dt*(self.current_iterate + self.N))].item()]]), self.theta])
