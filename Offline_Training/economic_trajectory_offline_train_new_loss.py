...
Training for economic and reference tracking is the same
But split in different files since they might require different hyperparameters
"""
seed = 42
random.seed(seed)
np.random.seed(seed)
...
csv_path = os.path.join(os.path.dirname(__file__), 'Training_Data/matched_residuals_thermal_management.csv')
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        net = self.net(x)
        #return neural network bounded by hyperbolic tangent
...
def main():
    ...
    for epoch in range(200):
        ...
        weight = 1.02
        # Rollout for T_rollout steps (simulate for T_rollout steps)
    ...
    # Evaluation, not necessary for training (comment if not requested)
    ...
    torch.save(residual_mlp.state_dict(), "Pretrained_Networks/REMOVE_TEST_SAVEeconomic_pretrain_FINAL.pth")
