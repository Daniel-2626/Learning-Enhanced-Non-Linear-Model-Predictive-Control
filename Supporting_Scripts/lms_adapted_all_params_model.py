...
# matrix containing all parameter adaptations
Theta = ca.SX.sym('Theta', 4, 1)

# column vector for storing initial state and target state
P = ca.SX.sym('P', n_states + n_states+4)
...
x1_dot_model = (k_model/(rho_model*A1_model)+Theta[0])*u_in - (a1_model/A1_model+Theta[1]) *ca.sqrt(2*grav_model*h_1+0.001)
x2_dot_model = (a1_model/A1_model+Theta[2]) *ca.sqrt(2*grav_model*h_1+0.001) - (a2_model/A2_model+Theta[3]) *ca.sqrt(2*grav_model*h_2+0.001)
ode_model = ca.vertcat(x1_dot_model, x2_dot_model)

f_model = ca.Function("f_model", [states,controls,Theta], [ode_model], ["x","u","theta"], ["ode"])
## Actual model
x1_dot = (k/(rho*A1))*u_in - a1/A1 *ca.sqrt(2*grav*h_1+0.001)
...
for k in range(N):
    ...
    st_next = X[:, k+1]
    K1 = f_model(st, contr,theta)
    K2 = f_model(st + step_horizon/2 * K1, contr,theta)
    K3 = f_model(st + step_horizon/2 * K2, contr,theta)
    K4 = f_model(st + step_horizon * K3, contr,theta)
    st_next_RK4 = st + (step_horizon / 6) * (K1 + 2*K2 + 2*K3 + K4)
    g = ca.vertcat(g, st_next - st_next_RK4) # Basically saying X[:, k+1] = runge kutta evaluation
print(type(st_next_RK4), st_next_RK4.shape)
def model_update(x0,u,theta, step_horizon, f_model):
    u0 = u[:,0]
    
    K1 = f_model(x0, u0,theta)
    K2 = f_model(x0 + step_horizon/2 * K1, u0, theta)
    K3 = f_model(x0 + step_horizon/2 * K2, u0, theta)
    K4 = f_model(x0 + step_horizon * K3, u0, theta)
    x0_plus_1 = ca.DM.full(x0 + (step_horizon / 6) * (K1 + 2*K2 + 2*K3 + K4))
    return x0_plus_1
...
if __name__ == '__main__':
    ...
    while (mpc_iter * step_horizon < sim_time):
        ...
        if mpc_iter > 1:
            ...
            mu = 1
            #print(x_actual.shape, x_model.shape)
            #print(G.shape)
            theta = theta + mu*(G.T)@(x_actual-x_model)
            #print(theta)
            theta_0 = theta[3,0]
           # print(theta_0)
        x_model = model_update(state_init, u, theta, step_horizon, f_model) #+ G@theta
        # In code, this is the actual measurement update
        t0, state_init, u0 = shift_timestep(step_horizon, t0, state_init, u, f_actual)
        
        x_actual = state_init

        print(x_model-x_actual)
        #print(x_actual,x_model)
        X0 = ca.horzcat(
            X0[:, 1:],
            ca.reshape(X0[:, -1], -1, 1)
        )
