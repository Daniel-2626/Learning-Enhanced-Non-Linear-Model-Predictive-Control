...
# L4CasADi expects state x and input u to be the same argument to nn
# However, for RK4 this will be inconvinient since we propagate state forward while keeping input constant
# This wrapper takes [x u] and splits this, so NN can be used in L4CasADi but also in RK4
class L4CWrapper(nn.Module):
    ...
    def forward(self, xu):
        x = xu[:, :2]
        u = xu[:, 2:]
        return self.torchnet(x,u)
### CASCADED TANKS DYNAMICS (learned , f_nom + f_res) ###
class CascadedTankLearnedDynamics:
    def __init__(self, residual_model, nominal_ratio): 
    ...
    def model(self):
        ...
        x_start = np.array([1,1]) # initial state (will be overwritten)
...
class MPC:
    ...
    def ocp(self):
        ...
        # Folders with autogenerated code 
...
h1_dot = k*u/(rho*A1)*cs.exp(-u/10) - a1/A1 * cs.sqrt(2*g*h1+0.00001) #<-- mismatched with hidden dynamics
h2_dot = a1/A1 * cs.sqrt(2*g*h1 + 0.00001) - a2/A2 * cs.sqrt(2*g*h2 + 0.00001) 
...
N = 50
t_horizon = 25
learned_model = CascadedTankLearnedDynamics(l4c_residual, nominal_ratio = nominal_ratio)
...
obs_buffer = []
batch_size = 50
T_update = 50 # every T_update steps NN is updated, however only if have collected data of size batch_size
nn_on = 0
...
for i in range(Steps):
    ...
    if i > 0 and (i % T_update) == 0 and len(obs_buffer) >= batch_size:
        ...
        #print(X_batch)
        # rollout length <-- for how long we simulate before we re-initialize 
        T_rollout = 10
        ...
        for epoch in range(100):
            ...
            for k0 in range(0, batch_size, T_rollout):
        ...
        l4c_wrapped = L4CWrapper(residual_mlp)
        l4c_residual.update(l4c_wrapped)
...
#df.to_csv("cascaded_adaptive_traj_mismatched.csv", index=False)
