...
h_1 = ca.SX.sym('h_1')
h_2 = ca.SX.sym('h_2')
h_1_int = ca.SX.sym('h_1_int')
h_2_int = ca.SX.sym('h_2_int')
states = ca.vertcat(
    h_1,
    h_2,
    h_1_int,
...
Q = ca.diagcat(Q_h_1, Q_h_2, Q_h_1_int, Q_h_2_int)
...
x3_dot_model = 0.7*(h_1 - h_1_target)
x4_dot_model = 0.7*(h_2 - h_2_target)
ode_model = ca.vertcat(x1_dot_model, x2_dot_model, x3_dot_model, x4_dot_model)
...
x3_dot = nominal_ratio*(h_1 - h_1_target)
x4_dot = nominal_ratio*(h_2 - h_2_target)
ode = ca.vertcat(x1_dot, x2_dot, x3_dot, x4_dot)
...
lbx[2: n_states*(N+1): n_states] = -ca.inf # Different notation to matlab. Here have [start:stop.step]. We fill out the vector related to states [start:n_states*(N+1)] with a lower bound every n_states, meaning we set lower bound for first state at each time step
lbx[3: n_states*(N+1): n_states] = -ca.inf # Here set lower bound for second state

ubx[0: n_states*(N+1): n_states] = h_1_max 
ubx[1: n_states*(N+1): n_states] = h_2_max 
ubx[2: n_states*(N+1): n_states] = ca.inf 
ubx[3: n_states*(N+1): n_states] = ca.inf 
...
t0 = 0
state_init = ca.DM([h_1_init, h_2_init, h_1_int_init, h_2_int_init]) # initial state
state_target = ca.DM([h_1_target, h_2_target, h_1_int_target, h_2_int_target])
...
fig.suptitle("Nominal integrative controller operating with mismatched (70%) model information", fontsize=16)
