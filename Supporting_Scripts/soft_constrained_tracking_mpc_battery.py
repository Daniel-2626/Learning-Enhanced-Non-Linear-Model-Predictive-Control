...
class MPC:
    ...
    def ocp(self):
        ...
        Tb_max = 22 + CELSIUS_TO_KELVIN
        Tb_min = 19 + CELSIUS_TO_KELVIN
        SOC_max = 1
        SOC_min = 0
        ...
        ocp.constraints.ubx_e = np.array([Tb_max])
        ocp.constraints.lbx_e = np.array([Tb_min])
        # Maybe will complain that constraint is a parameter
        ocp.constraints.idxsbx_e = np.array(range(nsx))
        ...
        ocp.cost.zl_e = 100 * np.zeros((nsx,))
        ocp.cost.zu_e = 100 * np.zeros((nsx,))
...
class Controller:
    ...
    def get_input(self, T_bat_target, T_bat_0, SOC_0, current_0, dT_bat, dSOC):
        ...
        pred = self.solver.get(200, 'x')
        
        print("Prediction of terminal temperature", pred[0].item())
